    ...
    $.ajax
      ...
      success: (re) ->
        ...
        if re.code == 0
          ...
          # $('#follow-tip').html('<p>零钱已入您的惠互动钱包！</p><p>识别二维码，关注公众号领取！</p>')
  ...
  commitPartinData: (token, eventId, payload, type) ->
    ...
    $.ajax
      ...
      success: (re) ->
        $('#commitBtn').button('reset')
        if re.code == 0
          window.location.href="/wx/event/take_result?id=#{eventId}&token=#{token}&code=0&log_id=#{re.data.id}"
        else
          window.location.href="/wx/event/take_result?id=#{eventId}&token=#{token}&code=1&message=#{re.message}"
      error: (error) ->
        $('#commitBtn').button('reset')
        alert(JSON.stringify(error))
        window.location.href="/wx/event/take_result?id=#{eventId}&token=#{token}&code=#{error.code || 500}"
...
#
  ...
  withdraw: (el, token) ->
    ...
    if parseFloat(money) <= 0
      # alert('余额不足，最低需要0.1元')
      App.alert("余额不足，最低需要0.1元", $('.withdraw-commit'))
      return
    
    account_no = $('.withdraw-commit #account').val()
    account_name = $('.withdraw-commit #name').val()
    
    if account_no.length == 0 or account_name.length == 0
      # alert('账号或姓名不能为空')
      App.alert("账号或姓名不能为空", $('.withdraw-commit'))
      return
    ...
    $.ajax
      ...
      type: "POST"
      data: { token: token, money: money, account_no: account_no, account_name: account_name, i: i, ak: ak }
      success: (re) ->
        $(el).button('reset')
        if re.code == 0
          # alert('成功提现到支付宝')
          # App.notice("成功提现到支付宝", $('.withdraw-commit'))
          window.location.href="/wx/wallet?msg=成功提现到支付宝&code=0"
        else
          # alert(re.message)
          # App.alert(re.message, $('.withdraw-commit'))
          window.location.href="/wx/wallet?msg=" + re.message + '&code=1'
      error: (err) ->
        $(el).button('reset')
        # alert('Oops, 服务器出错了，我们正在处理...')
        # App.alert('Oops, 服务器出错了，我们正在处理...', $('.withdraw-commit'))
        window.location.href="/wx/wallet?msg=" + 'Oops, 服务器出错了，我们正在处理...' + '&code=-1'

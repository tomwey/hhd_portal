module API
  module CommHelpers
    ...
    # 是否需要隐藏非法功能
    def need_hide_for_ios_approve
      return false if (params[:is_ios] || 0).to_i == 0
      
      return false if SiteConfig.in_approve_version.blank?
      
      return false if params[:av].blank?
      
      return params[:av] >= SiteConfig.in_approve_version
    end
